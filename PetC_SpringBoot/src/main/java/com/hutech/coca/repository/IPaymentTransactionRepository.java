...
public interface IPaymentTransactionRepository extends JpaRepository<PaymentTransaction, Long> {
    ...
    @Query("SELECT COALESCE(SUM(CASE WHEN (t.amount - COALESCE(t.voucherDiscount, 0)) < 0 THEN 0 ELSE (t.amount - COALESCE(t.voucherDiscount, 0)) END), 0) FROM PaymentTransaction t " +
    ...
    @Query("SELECT FUNCTION('DATE', t.updatedAt), " +
           "COALESCE(SUM(CASE WHEN (t.amount - COALESCE(t.voucherDiscount, 0)) < 0 THEN 0 ELSE (t.amount - COALESCE(t.voucherDiscount, 0)) END), 0) " +
    ...
    /** Doanh thu theo phương thức thanh toán */
       @Query("SELECT t.paymentMethod, COALESCE(SUM(CASE WHEN (t.amount - COALESCE(t.voucherDiscount, 0)) < 0 THEN 0 ELSE (t.amount - COALESCE(t.voucherDiscount, 0)) END), 0) " +
           "FROM PaymentTransaction t " +
           "WHERE t.paymentStatus = 'SUCCESS' AND t.updatedAt >= :from AND t.updatedAt < :to " +
