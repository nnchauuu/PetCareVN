...
public class RevenueController {
    ...
    /**
     * GET /api/admin/revenue/cancelled-bookings?from=2026-03-01&to=2026-03-31
     * Danh sách booking đã hủy trong kỳ
     */
    @GetMapping("/cancelled-bookings")
    public ResponseEntity<Map<String, Object>> getCancelledBookings(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        try {
            List<Map<String, Object>> data = revenueService.getCancelledBookings(from, to);
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("data", data);
            return ResponseEntity.ok(response);
