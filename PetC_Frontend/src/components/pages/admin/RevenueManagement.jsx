...
const RevenueManagement = () => {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            const [summaryRes, chartRes, txRes, cancelledRes] = await Promise.all([
                ...
                fetch(`${API}/api/admin/revenue/cancelled-bookings?from=${from}&to=${to}`, { headers }),
            ]);

            const [summaryJson, chartJson, txJson, cancelledJson] = await Promise.all([
                summaryRes.json(), chartRes.json(), txRes.json(), cancelledRes.json(),
            ]);

            if (summaryJson.success) setSummary(summaryJson.data);
            if (chartJson.success) setChartData(chartJson.data);
            if (txJson.success) setTransactions(txJson.data);
            if (cancelledJson.success) setCancelledBookings(cancelledJson.data);
        } catch (e) {
    ...
    return (
        <div>
                                ...
                                <tbody>
                                    {transactions.map((tx, idx) => (
                                        <tr key={tx.id}>
                                            ...
                                            <td className="text-end fw-bold text-success">{formatVnd(Math.max(0, tx.netRevenue || 0))}</td>
                                ...
                                <tfoot className="table-light">
                                    <tr>
                                        ...
                                        <td className="text-end fw-bold text-success">
                                            {formatVnd(transactions.reduce((s, tx) => s + Math.max(0, tx.netRevenue || 0), 0))}
                                        </td>
                                        <td></td>
                                    </tr>
                                </tfoot>
            ...
            {summary && (
                <div className="card border-0 shadow-sm mt-4">
                    <div className="card-body d-flex justify-content-between align-items-center">
                        <div>
                            <h6 className="text-muted mb-1">Đơn hủy trong kỳ</h6>
                            <h4 className="mb-0 text-danger fw-bold">{summary.cancelledBookings ?? 0}</h4>
                        </div>
                        <div className="bg-danger bg-opacity-10 p-3 rounded">
                            <i className="fas fa-ban text-danger fs-3"></i>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="card border-0 shadow-sm mt-4">
                <div className="card-header bg-white d-flex justify-content-between align-items-center">
                    <span className="fw-semibold">Danh sách đơn đã hủy</span>
                    <span className="badge bg-danger">{cancelledBookings.length} đơn</span>
                </div>
                <div className="card-body p-0">
                    {cancelledBookings.length === 0 ? (
                        <div className="text-center text-muted py-4">Không có đơn hủy trong kỳ này.</div>
                    ) : (
                        <div className="table-responsive">
                            <table className="table table-hover align-middle mb-0">
                                <thead className="table-light">
                                    <tr>
                                        ...
                                        <th>Khách hàng</th>
                                        <th>Email</th>
                                        <th className="text-end">Giá trị booking</th>
                                        <th>Lịch hẹn</th>
                                ...
                                <tbody>
                                    {cancelledBookings.map((booking, idx) => (
                                        <tr key={booking.id}>
                                            ...
                                            <td className="text-muted">{booking.customerEmail || '-'}</td>
                                            <td className="text-end">{formatVnd(booking.totalPrice || 0)}</td>
                                            <td className="text-muted">{formatDate(booking.scheduledAt)}</td>
                                            <td className="text-muted">{formatDate(booking.cancelledAt)}</td>
